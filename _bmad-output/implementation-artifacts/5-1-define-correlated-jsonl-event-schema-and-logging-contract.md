...
- [ ] Strengthen and finalize the event schema contract in `src/infrastructure/logging/event_schema.py` (AC: 1, 3)
  - [ ] Enforce required fields with deterministic validation errors
  - [ ] Validate UTC ISO-8601 timestamps and keep behavior stable for nullable optional fields
  - [ ] Enforce `domain.action` naming consistently and document edge conditions
- [ ] Harden JSONL writer behavior in `src/infrastructure/logging/jsonl_logger.py` (AC: 2, 4)
  - [ ] Guarantee append-only one-JSON-object-per-line persistence
  - [ ] Ensure logger failures are returned/raised as structured local failures without UI-thread crash propagation
  - [ ] Preserve local-only behavior (no network transport path)
- [ ] Align pipeline emitters with the schema contract across services/adapters (AC: 1, 3, 4)
  - [ ] Confirm stage/event/severity payload quality in extraction, orchestration, playback, and UI presenter paths
  - [ ] Normalize any non-conformant event names or missing required fields
- [ ] Extend automated verification for schema + append-only behavior (AC: 1, 2, 3, 4)
  - [ ] Add/expand unit tests for schema validator success/failure cases
  - [ ] Add/expand integration coverage validating emitted JSONL lines from bootstrap and runtime flows
